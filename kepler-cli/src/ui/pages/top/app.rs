...
use crate::ui::widgets::date_range_picker::{DateRangePickerAction, DateRangePickerState};
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum LogFocus {
    /// No widget focused — Left/Right/Tab/BackTab switch between main tabs
    None,
    Chart,
    Filter,
    Viewer,
...
pub struct TopApp {
    ...
    /// Pre-computed aggregate of all service histories, updated when data changes.
    pub total_history: VecDeque<MonitorMetricEntry>,
    ...
    pub metrics_refetch_at: Option<std::time::Instant>,
    /// Keyboard cursor on dashboard charts (timestamp ms), shown on both CPU & Memory
    pub dashboard_chart_cursor: Option<i64>,
    /// Keyboard selection on dashboard charts (start_ts, end_ts), shown on both CPU & Memory
    pub dashboard_chart_selection: Option<(i64, i64)>,
    ...
    /// Maps each visual row in the log viewer to an entry ID (for click detection with wrapping)
    pub log_viewer_row_entry_ids: Vec<i64>,
    ...
    pub log_detail_width_ratio: f32,
    pub dragging_detail_border: bool,
    pub log_detail_border_x: Option<u16>,
    /// Clickable button areas in the detail panel: (action_name, area)
    pub log_detail_button_areas: Vec<(&'static str, Rect)>,
    ...
    // --- Date range picker ---
    pub date_range_picker: Option<DateRangePickerState>,
    pub range_button_area: Option<Rect>,

    // --- User rights ---
    pub user_rights: HashSet<String>,
}
...
impl TopApp {
    pub fn new(interval: Duration, initial_service: Option<String>) -> Self {
        Self {
            ...
            log_viewer_row_entry_ids: Vec::new(),
            log_focus: LogFocus::None,
            ...
            log_detail_width_ratio: 0.40,
            dragging_detail_border: false,
            log_detail_border_x: None,
            log_detail_button_areas: Vec::new(),
            mouse_pos: (0, 0),
            last_click: None,
            date_range_picker: None,
            range_button_area: None,
            user_rights: HashSet::new(),
        }
    }

    pub fn has_right(&self, right: &str) -> bool {
        // Check this right and all parent rights in the hierarchy.
        // e.g. "logs:search:sql" requires "logs:search" which requires "logs".
        if !self.user_rights.contains(right) {
            return false;
        }
        let mut prefix = right;
        while let Some(pos) = prefix.rfind(':') {
            prefix = &prefix[..pos];
            if !self.user_rights.contains(prefix) {
                return false;
            }
        }
        true
    ...
    pub fn available_tabs(&self) -> Vec<Tab> {
        ...
        tabs.push(Tab::Detail);
        tabs
    }

    pub fn available_menu_items(&self) -> Vec<(&'static str, usize)> {
        let mut items = vec![("Focus", 0)];
        if self.has_right("start") { items.push(("Start", 1)); }
        if self.has_right("stop") { items.push(("Stop", 2)); }
        if self.has_right("restart") { items.push(("Restart", 3)); }
        items
    }
    ...
    pub fn switch_tab(&mut self, tab: Tab) {
        ...
        // Reset focus states
        self.dashboard_focus = DashboardFocus::None;
        self.dashboard_chart_cursor = None;
        self.dashboard_chart_selection = None;
        // Reset detail panel
    ...
    /// Start services (no confirmation needed).
    pub fn do_start_services(&mut self) {
        if let Some(targets) = self.action_target_services() {
            ...
            self.pending_actions.push(AppAction::StartServices(targets));
            self.set_feedback(format!("Starting {}...", label), FeedbackKind::Info);
        }
    }

    /// Stop services (with confirmation dialog).
    pub fn do_stop_services(&mut self) {
        if let Some(targets) = self.action_target_services() {
    ...
    /// Restart services (with confirmation dialog).
    pub fn do_restart_services(&mut self) {
        if let Some(targets) = self.action_target_services() {
    ...
    /// Returns the timestamp range of the log entry buffer, or None if empty.
    pub fn log_buffer_range(&self) -> Option<(i64, i64)> {
        let first = self.log_entries.front()?.timestamp;
        let last = self.log_entries.back()?.timestamp;
        Some((first, last))
    }

    /// Schedule a debounced log refetch (300ms from now).
    /// Each call resets the timer so rapid panning only triggers one fetch.
    /// Skips scheduling if the current visible range is fully within the log buffer
    /// (logs are not sampled, so zooming in on buffered data needs no refetch).
    pub fn schedule_log_refetch(&mut self) {
        let right_edge_ms = self.current_right_edge_ms();
        let window_ms = self.chart_window.as_millis() as i64;
        let left_edge_ms = right_edge_ms - window_ms;

        if let Some((buf_left, buf_right)) = self.log_buffer_range() {
            if left_edge_ms >= buf_left && right_edge_ms <= buf_right {
                return;
            }
        }
    ...
    /// Schedule a debounced metrics refetch (300ms from now).
    /// Always refetches so the server can resample at the appropriate resolution.
    pub fn schedule_metrics_refetch(&mut self) {
        self.metrics_refetch_at = Some(std::time::Instant::now() + Duration::from_millis(300));
    }

    /// Schedule both log and metrics refetches. Since the time range is shared
    /// across all charts, any pan/zoom operation should refresh both data sources.
    pub fn schedule_data_refetch(&mut self) {
        self.schedule_log_refetch();
        self.schedule_metrics_refetch();
    }

    /// Open the date range picker popup, initialized with the current chart viewport.
    pub fn open_date_range_picker(&mut self) {
        let right_ms = self.current_right_edge_ms();
        let left_ms = right_ms - self.chart_window.as_millis() as i64;
        self.date_range_picker = Some(DateRangePickerState::new(left_ms, right_ms));
        self.overlay = Overlay::DateRangePicker;
    }

    /// Handle the result of a date range picker action.
    fn handle_date_range_picker_action(&mut self, action: DateRangePickerAction) {
        match action {
            DateRangePickerAction::Applied(start_ms, end_ms) => {
                let window_ms = (end_ms - start_ms).max(1000);
                self.chart_window = Duration::from_millis(window_ms as u64);
                self.chart_mode = ChartMode::Paused { right_edge_ms: end_ms };
                self.overlay = Overlay::None;
                self.date_range_picker = None;
                self.schedule_data_refetch();
            }
            DateRangePickerAction::Cancelled => {
                self.overlay = Overlay::None;
                self.date_range_picker = None;
            }
    ...
    /// Close the log detail panel and return focus to the viewer.
    ...
    pub fn show_tooltip_at(&mut self, col: u16, row: u16, chart_kind: ChartKind) {
        ...
        if self.is_all_view() && !self.services.is_empty() {
            let points = utils::chart_data_points(&self.total_history, self.chart_window, right_edge_ms, extractor);
    ...
    fn show_log_tooltip_at(&mut self, col: u16, row: u16) {
        ...
        // Count per category: error, warn, info, debug, trace
        ...
        let colors = [self.theme.error, self.theme.warning, self.theme.accent, self.theme.teal, self.theme.fg_dim];

        let mut entries = Vec::new();
        for i in 0..5 {
            if counts[i] > 0 {
                entries.push((names[i].to_string(), counts[i] as f64, colors[i]));
            }
        }

        let total: u32 = counts.iter().sum();
        if total > 0 {
            entries.push(("total".to_string(), total as f64, self.theme.fg_bright));
        }
    ...
    /// Compute the approximate data area for a dashboard chart (CPU or Memory).
    /// ratatui Chart uses ~8 chars left margin (y-axis) and ~2 chars right margin.
    pub fn dashboard_chart_data_area(&self, chart_area: Rect) -> Option<Rect> {
        let y_label_width: u16 = 8;
        let right_margin: u16 = 2;
        let data_width = chart_area.width.saturating_sub(y_label_width + right_margin);
        let data_height = chart_area.height.saturating_sub(3); // border top, x-axis label, border bottom
        if data_width == 0 || data_height == 0 {
            return None;
        }
        Some(Rect::new(
            chart_area.x + y_label_width,
            chart_area.y + 1,
            data_width,
            data_height,
        ))
    ...
    /// Convert a timestamp to a column index within a dashboard chart data area.
    pub fn dashboard_ts_to_col(&self, ts: i64, data_width: u16) -> u16 {
        let right_edge_ms = self.current_right_edge_ms();
        let window_ms = self.chart_window.as_millis() as i64;
        let left_edge_ms = right_edge_ms - window_ms;
        if window_ms == 0 {
        ...
        let frac = (ts - left_edge_ms) as f64 / window_ms as f64;
        let col = (frac * data_width as f64).round() as i64;
        col.max(0).min(data_width as i64 - 1) as u16
    }

    /// Convert a column index within a dashboard chart data area to a timestamp.
    pub fn dashboard_col_to_ts(&self, col: u16, data_width: u16) -> i64 {
        let right_edge_ms = self.current_right_edge_ms();
        let window_ms = self.chart_window.as_millis() as i64;
        let left_edge_ms = right_edge_ms - window_ms;
        left_edge_ms + (col as i64 * window_ms) / data_width.max(1) as i64
    }

    /// Convert dashboard chart cursor timestamp to a column index.
    pub fn dashboard_cursor_col(&self, data_width: u16) -> Option<u16> {
        self.dashboard_chart_cursor.map(|ts| self.dashboard_ts_to_col(ts, data_width))
    }

    /// Convert dashboard chart selection timestamps to column indices.
    pub fn dashboard_selection_cols(&self, data_width: u16) -> Option<(u16, u16)> {
        self.dashboard_chart_selection.map(|(start, end)| {
            (self.dashboard_ts_to_col(start, data_width), self.dashboard_ts_to_col(end, data_width))
        })
    }
    ...
    fn handle_key(&mut self, key: KeyEvent) {
        ...
        match &self.overlay {
            ...
            Overlay::ContextMenu { service, selected, .. } => {
                ...
                let sel = *selected;
                let menu_items = self.available_menu_items();
                match key.code {
                    KeyCode::Esc => {
                        self.overlay = Overlay::None;
                    }
                    ...
                    KeyCode::Enter => {
                        self.overlay = Overlay::None;
                        if sel < menu_items.len() {
                            self.execute_menu_action(&service, menu_items[sel].1);
                        }
                    }
                    _ => {}
                }
            ...
            Overlay::DateRangePicker => {
                if let Some(ref mut picker) = self.date_range_picker {
                    let action = picker.handle_key(key);
                    self.handle_date_range_picker_action(action);
                }
                return;
            }
            Overlay::None => {}
        ...
        if self.tab == Tab::Logs && self.log_focus == LogFocus::Chart {
            match key.code {
                ...
                KeyCode::Tab => {
                    self.log_focus = if self.has_right("logs:search") {
                        LogFocus::Filter
                    } else {
                        LogFocus::Viewer
                    };
        ...
        if self.tab == Tab::Logs && self.log_focus == LogFocus::Filter {
            ...
            if key.modifiers.contains(KeyModifiers::CONTROL) {
                match key.code {
                    ...
                    KeyCode::Char('t') => {
                        if self.has_right("logs:search:sql") {
        ...
        if self.tab == Tab::Logs && self.log_focus == LogFocus::Viewer {
            match key.code {
                ...
                KeyCode::Char('+') | KeyCode::Char('=') => {
                    self.buffer_size = (self.buffer_size + 1000).min(100_000);
                    return;
                }
                KeyCode::Char('-') => {
                    self.buffer_size = self.buffer_size.saturating_sub(1000).max(1000);
                    return;
                }
                KeyCode::Char('0') => {
                    self.chart_window = Duration::from_secs(300);
        ...
        // Dashboard tab with chart focus: dedicated controls
        if self.tab == Tab::Dashboard && self.dashboard_focus != DashboardFocus::None {
            let chart_kind = match self.dashboard_focus {
                DashboardFocus::Cpu => ChartKind::Cpu,
                DashboardFocus::Memory => ChartKind::Memory,
                DashboardFocus::None => unreachable!(),
            };
            // Use CPU chart area for data width computation (both charts are equal width)
            let ref_area = self.cpu_chart_area;
            let data_width = ref_area.and_then(|a| self.dashboard_chart_data_area(a))
                .map(|d| d.width)
                .unwrap_or(1);
            match key.code {
                KeyCode::Char('q') => { self.should_quit = true; return; }
                ...
                KeyCode::Tab => {
                    self.dashboard_focus = match self.dashboard_focus {
                        DashboardFocus::Cpu => DashboardFocus::Memory,
                        DashboardFocus::Memory => DashboardFocus::None,
                        DashboardFocus::None => DashboardFocus::Cpu,
                    };
                    self.tooltip = None;
                    return;
                }
                KeyCode::BackTab => {
                    self.dashboard_focus = match self.dashboard_focus {
                        DashboardFocus::Cpu => DashboardFocus::None,
                        DashboardFocus::Memory => DashboardFocus::Cpu,
                        DashboardFocus::None => DashboardFocus::Memory,
                    };
                    self.tooltip = None;
                    return;
                }
                ...
                KeyCode::Char('i') => {
                    if self.tooltip.is_some() {
                        self.tooltip = None;
                    } else if let Some(cursor_ts) = self.dashboard_chart_cursor {
                        // Show tooltip at cursor position on the focused chart
                        let chart_area = match chart_kind {
                            ChartKind::Cpu => self.cpu_chart_area,
                            ChartKind::Memory => self.memory_chart_area,
                            _ => None,
                        ...
                        if let Some(area) = chart_area {
                            if let Some(data_area) = self.dashboard_chart_data_area(area) {
                                let col = self.dashboard_ts_to_col(cursor_ts, data_area.width);
                                let screen_col = data_area.x + col;
                                let screen_row = data_area.y + 1;
                                self.show_tooltip_at(screen_col, screen_row, chart_kind);
                            }
                        }
                    } else {
                        // No cursor — show tooltip at center of focused chart
                        let chart_area = match chart_kind {
                            ChartKind::Cpu => self.cpu_chart_area,
                            ChartKind::Memory => self.memory_chart_area,
                            _ => None,
                        ...
                        if let Some(area) = chart_area {
                            ...
                            self.show_tooltip_at(col, row, chart_kind);
                        }
                    }
                    return;
                }
                KeyCode::Left if key.modifiers.contains(KeyModifiers::SHIFT) => {
                    ...
                    let cursor_col = self.dashboard_cursor_col(data_width)
                        .unwrap_or(data_width.saturating_sub(1));
                    if cursor_col > 0 {
                        let next = cursor_col - 1;
                        let new_ts = self.dashboard_col_to_ts(next, data_width);
                        if let Some((start_ts, _end)) = self.dashboard_chart_selection {
                            self.dashboard_chart_cursor = Some(new_ts);
                            self.dashboard_chart_selection = Some((start_ts, new_ts));
                        } else {
                            let start_ts = self.dashboard_col_to_ts(cursor_col, data_width);
                            self.dashboard_chart_selection = Some((start_ts, new_ts));
                            self.dashboard_chart_cursor = Some(new_ts);
                        }
                    }
                ...
                KeyCode::Left => {
                    ...
                    let cursor_col = self.dashboard_cursor_col(data_width)
                        .unwrap_or(data_width.saturating_sub(1));
                    if cursor_col > 0 {
                        self.dashboard_chart_cursor = Some(self.dashboard_col_to_ts(cursor_col - 1, data_width));
                    } else {
                        // Pan left
                        let step_ms = self.chart_window.as_millis() as i64 / data_width.max(1) as i64;
                        if matches!(self.chart_mode, ChartMode::Live) {
                            let now = chrono::Utc::now().timestamp_millis();
                            self.chart_mode = ChartMode::Paused { right_edge_ms: now };
                        }
                        if let ChartMode::Paused { right_edge_ms } = &mut self.chart_mode {
                            *right_edge_ms -= step_ms;
                        }
                        self.dashboard_chart_cursor = Some(self.dashboard_col_to_ts(0, data_width));
                        self.schedule_data_refetch();
                    }
                    self.dashboard_chart_selection = None;
                    return;
                ...
                KeyCode::Right if key.modifiers.contains(KeyModifiers::SHIFT) => {
                    ...
                    let max_col = data_width.saturating_sub(1);
                    let cursor_col = self.dashboard_cursor_col(data_width).unwrap_or(0);
                    if cursor_col < max_col {
                        let next = cursor_col + 1;
                        let new_ts = self.dashboard_col_to_ts(next, data_width);
                        if let Some((start_ts, _end)) = self.dashboard_chart_selection {
                ...
                KeyCode::Right => {
                    ...
                    if cursor_col < max_col {
                        ...
                        if let ChartMode::Paused { right_edge_ms } = &mut self.chart_mode {
                            *right_edge_ms = (*right_edge_ms + step_ms).min(now_ms);
                        }
                        self.dashboard_chart_cursor = Some(self.dashboard_col_to_ts(max_col, data_width));
                        self.schedule_data_refetch();
                    }
                    self.dashboard_chart_selection = None;
                    return;
                ...
                KeyCode::Enter => {
                    if let Some((start_ts, end_ts)) = self.dashboard_chart_selection {
                        // Zoom to selection
                        let t_left = start_ts.min(end_ts);
                        let t_right = start_ts.max(end_ts);
                        let new_window = (t_right - t_left).max(5000);
                        self.chart_window = Duration::from_millis(new_window as u64);
                        self.chart_mode = ChartMode::Paused { right_edge_ms: t_right };
                        self.dashboard_chart_selection = None;
                        ...
                        // Zoom to single column at cursor
                        let step_ms = self.chart_window.as_millis() as i64 / data_width.max(1) as i64;
                        let new_window = step_ms.max(5000);
                        self.chart_window = Duration::from_millis(new_window as u64);
                        self.chart_mode = ChartMode::Paused { right_edge_ms: cursor_ts + step_ms };
                        self.dashboard_chart_selection = None;
                        self.dashboard_chart_cursor = None;
                        self.schedule_data_refetch();
                    }
                ...
                KeyCode::End => {
                    let last_col = data_width.saturating_sub(1);
                    self.dashboard_chart_cursor = Some(self.dashboard_col_to_ts(last_col, data_width));
                    self.dashboard_chart_selection = None;
                    return;
                }
                KeyCode::Esc => {
                    if self.tooltip.is_some() {
                        self.tooltip = None;
                    } else if self.dashboard_chart_selection.is_some() {
                        self.dashboard_chart_selection = None;
                    } else if self.dashboard_chart_cursor.is_some() {
                        self.dashboard_chart_cursor = None;
                    } else {
                        self.dashboard_focus = DashboardFocus::None;
                    }
        ...
        match key.code {
            ...
            KeyCode::Tab => {
                ...
                } else if self.tab == Tab::Logs && self.log_focus == LogFocus::None {
                    self.log_focus = LogFocus::Chart;
                } else {
                    let tabs = self.available_tabs();
                    let idx = tabs.iter().position(|t| *t == self.tab).unwrap_or(0);
            ...
            // +/- : adjust buffer size (Logs & Dashboard with no focus)
            // Ctrl++/Ctrl+- : adjust refresh rate
            KeyCode::Char('+') | KeyCode::Char('=') => {
                if key.modifiers.contains(KeyModifiers::CONTROL) {
                ...
                } else if self.tab == Tab::Logs || self.tab == Tab::Dashboard {
                    self.buffer_size = (self.buffer_size + 1000).min(100_000);
                }
            }
            KeyCode::Char('-') => {
            ...
            // Escape: unfocus service / clear toggles (never quits — use 'q')
            KeyCode::Esc => {
            ...
            KeyCode::Char('s') => {
                if self.has_right("start") {
                    self.do_start_services();
                }
            }

            // Service management: stop (with confirmation)
            KeyCode::Char('S') => {
                if self.has_right("stop") {
                    self.do_stop_services();
                }
            }

            // Service management: restart (with confirmation)
            KeyCode::Char('r') => {
                if self.has_right("restart") {
    ...
    fn handle_mouse(&mut self, mouse: MouseEvent) {
        ...
        match mouse.kind {
            MouseEventKind::Down(MouseButton::Left) => {
                ...
                // Check range button click
                if let Some(range_area) = self.range_button_area {
                    if mouse.column >= range_area.x
                        && mouse.column < range_area.x + range_area.width
                        && mouse.row >= range_area.y
                        && mouse.row < range_area.y + range_area.height
                    {
                        self.open_date_range_picker();
                        return;
                    }
                ...
                if self.tab == Tab::Detail {
                    ...
                    if let Some(action) = clicked_action {
                        match action {
                            AppAction::StartServices(_) => self.do_start_services(),
                            AppAction::StopService(_) | AppAction::StopAll => self.do_stop_services(),
                            AppAction::RestartServices(_) => self.do_restart_services(),
                        }
                        return;
                    }
                }
                ...
                // Check log detail panel border drag
                if let Some(border_x) = self.log_detail_border_x {
                    if mouse.column >= border_x.saturating_sub(1)
                        && mouse.column <= border_x + 1
                    {
                        self.dragging_detail_border = true;
                        return;
                    }
                }
                ...
                // Check click on log detail panel area — focus detail + buttons + select/copy
                if self.tab == Tab::Logs && self.log_detail_open {
                    if let Some(panel_area) = self.log_detail_panel_area {
                            ...
                            && mouse.row < panel_area.y + panel_area.height
                        {
                            self.log_focus = LogFocus::Detail;
                            // Check button clicks
                            for (action, rect) in &self.log_detail_button_areas {
                                if mouse.column >= rect.x
                                    && mouse.column < rect.x + rect.width
                                    && mouse.row >= rect.y
                                    && mouse.row < rect.y + rect.height
                ...
                if self.tab == Tab::Logs {
                    if let Some(viewer_area) = self.log_viewer_area {
                        ...
                        {
                            self.log_focus = LogFocus::Viewer;
                            // Use row-to-entry mapping (accounts for wrapping)
                            let row_in_viewer = (mouse.row - viewer_area.y) as usize;
                            if let Some(&entry_id) = self.log_viewer_row_entry_ids.get(row_in_viewer) {
                                ...
                                if is_double {
                                    let filtered = self.log_filtered_entries();
                                    if let Some(idx) = filtered.iter().position(|e| e.id == entry_id) {
                                        self.open_log_detail(idx);
                                    }
                                }
                                self.last_click = Some((mouse.column, mouse.row, now_inst));
                            }
                        }
                    }
                }
            ...
            MouseEventKind::Up(MouseButton::Left) => {
                ...
                self.dragging_detail_border = false;
            ...
            MouseEventKind::Up(MouseButton::Middle) => {
                if let Some(drag) = self.drag_state.take() {
                    // Re-fetch data only if the new visible range extends beyond the buffer
                    let right_edge_ms = self.current_right_edge_ms();
                    let window_ms = self.chart_window.as_millis() as i64;
                    let left_edge_ms = right_edge_ms - window_ms;

                    // For log data: clear buffer immediately if out of range
                    if drag.chart_kind == ChartKind::LogLevel {
                        let needs_log_refetch = match self.log_buffer_range() {
                            Some((buf_left, buf_right)) => {
                                left_edge_ms < buf_left || right_edge_ms > buf_right
                            }
                            None => true,
                        };
                        if needs_log_refetch {
                            self.log_last_id = None;
                            self.log_entries.clear();
                            self.log_scroll = 0;
                        }
                    }
                    // Always schedule both refetches (each checks its own buffer range)
                    self.schedule_data_refetch();
                }
            }
            MouseEventKind::Drag(MouseButton::Left) => {
                ...
                } else if self.dragging_detail_border {
                    // Horizontal border drag (detail panel width)
                    if let Some(content_area) = self.content_area {
                        let logs_tab_width = content_area.width;
                        if logs_tab_width > 0 {
                            let right_edge = content_area.x + content_area.width;
                            let ratio = (right_edge - mouse.column) as f32 / logs_tab_width as f32;
                            self.log_detail_width_ratio = ratio.clamp(0.15, 0.70);
                        }
                    }
            ...
            MouseEventKind::ScrollUp => {
                // Scroll on refresh area → increase interval
                if let Some(area) = self.refresh_area {
                ...
                // Scroll on buffer area → increase buffer size
                if let Some(area) = self.buffer_area {
                    if mouse.column >= area.x && mouse.column < area.x + area.width
                        && mouse.row >= area.y && mouse.row < area.y + area.height
                    {
                        self.buffer_size = (self.buffer_size + 1000).min(100_000);
            ...
            MouseEventKind::ScrollDown => {
                // Scroll on refresh area → decrease interval
                if let Some(area) = self.refresh_area {
                    if mouse.column >= area.x && mouse.column < area.x + area.width
                        && mouse.row >= area.y && mouse.row < area.y + area.height
                    {
                ...
                // Scroll on buffer area → decrease buffer size
                if let Some(area) = self.buffer_area {
                    if mouse.column >= area.x && mouse.column < area.x + area.width
                        && mouse.row >= area.y && mouse.row < area.y + area.height
                    {
    ...
    pub fn update_metrics(&mut self, entries: Vec<MonitorMetricEntry>) {
        for entry in entries {
            ...
            while history.len() > self.buffer_size {
                history.pop_front();
            }
    ...
    pub fn append_log_entries(&mut self, entries: Vec<DisplayLogEntry>) {
        ...
        while self.log_entries.len() > self.buffer_size {
    ...
    /// Rebuild the pre-computed total history from all service histories.
    /// Called after data changes (update_metrics, refetch, restore).
    pub fn rebuild_total_history(&mut self) {
        let mut totals: std::collections::BTreeMap<i64, MonitorMetricEntry> = std::collections::BTreeMap::new();
        for history in self.history.values() {
            for entry in history {
                totals.entry(entry.timestamp)
                    .and_modify(|t| {
                        t.cpu_percent += entry.cpu_percent;
                        t.memory_rss += entry.memory_rss;
                        t.memory_vss += entry.memory_vss;
                    })
                    .or_insert_with(|| MonitorMetricEntry {
                        timestamp: entry.timestamp,
                        service: "Total".to_string(),
                        cpu_percent: entry.cpu_percent,
                        memory_rss: entry.memory_rss,
                        memory_vss: entry.memory_vss,
                        pids: Vec::new(),
                    });
            }
        }
        self.total_history = totals.into_values().collect();
    }
}
