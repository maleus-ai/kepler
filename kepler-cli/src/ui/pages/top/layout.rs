...
use super::app::{DashboardFocus, LogFocus, TopApp};
...
pub fn render(f: &mut Frame, app: &mut TopApp) {
    ...
    let tab_labels: Vec<(Tab, &str)> = app.available_tabs().iter().map(|t| match t {
        Tab::Dashboard => (*t, "Dashboard"),
        Tab::Logs => (*t, "Logs"),
        Tab::Detail => (*t, "Detail"),
    }).collect();
    let (tab_rects, indicator_rect, refresh_rect, buffer_rect, range_rect) = widgets::tab_bar::render(f, tab_area, app.tab, &tab_labels, app.chart_mode, app.interval.as_secs(), app.buffer_size, app.chart_window, app.mouse_pos, &app.theme);
    app.tab_areas = tab_rects;
    app.live_indicator_area = indicator_rect;
    app.refresh_area = refresh_rect;
    app.buffer_area = buffer_rect;
    app.range_button_area = range_rect;
    ...
    match &app.overlay {
        ...
        Overlay::ContextMenu {
            ...
            let menu_items: Vec<&str> = app.available_menu_items().iter().map(|(label, _)| *label).collect();
            widgets::context_menu::render(f, &app.theme, *x, *y, *selected, &menu_items);
        }
        Overlay::DateRangePicker => {
            if let Some(ref mut picker) = app.date_range_picker {
...
fn render_dashboard(f: &mut Frame, area: Rect, app: &mut TopApp) {
    ...
    let cpu_focused = app.dashboard_focus == DashboardFocus::Cpu;
    let mem_focused = app.dashboard_focus == DashboardFocus::Memory;
    widgets::cpu_chart::render(f, chart_chunks[0], app, cpu_focused);
    widgets::memory_chart::render(f, chart_chunks[1], app, mem_focused);

    // Helper to highlight columns on a chart (paints bg_highlight on non-data cells)
    let highlight_chart_columns = |f: &mut Frame, chart_rect: Rect, x_start: u16, x_end: u16, theme: &crate::ui::theme::Theme| {
        let buf = f.buffer_mut();
        for x in x_start..x_end {
            for y in chart_rect.y..chart_rect.y + chart_rect.height {
    ...
    // Render zoom selection overlay on BOTH charts if dragging on a dashboard chart
    if let Some(ref zoom) = app.zoom_state {
        if zoom.is_real_drag && zoom.chart_kind != super::app::ChartKind::LogLevel {
            ...
            for chart_rect in [chart_chunks[0], chart_chunks[1]] {
                let x = col_min.max(chart_rect.x);
                let x_end = col_max.min(chart_rect.x + chart_rect.width);
                if x < x_end {
                    highlight_chart_columns(f, chart_rect, x, x_end, &app.theme);
                }
            }
        }
    }

    // Render keyboard cursor and selection on BOTH charts
    for chart_rect in [chart_chunks[0], chart_chunks[1]] {
        if let Some(data_area) = app.dashboard_chart_data_area(chart_rect) {
            // Render cursor
            if let Some(cursor_col) = app.dashboard_cursor_col(data_area.width) {
                let x = data_area.x + cursor_col;
                if x < data_area.x + data_area.width {
                    highlight_chart_columns(f, chart_rect, x, x + 1, &app.theme);
                }
            }

            // Render selection
            if let Some((start, end)) = app.dashboard_selection_cols(data_area.width) {
                ...
                if x_start < x_end {
                    highlight_chart_columns(f, chart_rect, x_start, x_end, &app.theme);
                }
            }
        }
    }
...
fn render_logs_tab(f: &mut Frame, area: Rect, app: &mut TopApp) {
    ...
    let (logs_area, detail_area) = if app.log_detail_open {
        let detail_width = ((area.width as f32 * app.log_detail_width_ratio) as u16)
            .clamp(30, area.width.saturating_sub(30).max(30));
        ...
        let h_chunks = Layout::default()
            ...
            .split(area);
        app.log_detail_border_x = Some(h_chunks[1].x);
        (h_chunks[0], Some(h_chunks[1]))
    } else {
        app.log_detail_border_x = None;
        (area, None)
    };
    ...
    let has_search = app.has_right("logs:search");

    // Compute dynamic filter height: borders (2) + visual content lines, capped
    let filter_height = if has_search {
        ...
        (visual_lines + 2).min(10) // borders + cap at 10 rows
    } else {
        0
    };
