...
use crate::ui::widgets::utils::{chart_data_points, format_bytes, format_duration_label, format_timestamp_label, nice_ceil_bytes};
...
pub fn render(f: &mut Frame, area: Rect, app: &TopApp, focused: bool) {
    ...
    for svc in &visible {
        if let Some(history) = app.history.get(*svc) {
            let points = chart_data_points(history, window, right_edge_ms, |e| e.memory_rss as f64);
            for &(_, y) in &points {
                if y > max_y {
                    max_y = y;
                }
    ...
    if app.is_all_view() && !app.services.is_empty() {
        let points = chart_data_points(&app.total_history, window, right_edge_ms, |e| e.memory_rss as f64);
        for &(_, y) in &points {
            if y > max_y {
                max_y = y;
            }
    ...
    // Round up to a nice byte-aligned value; minimum 1 MB
    max_y = nice_ceil_bytes(max_y * 1.2, 1024.0 * 1024.0);
    ...
    let y_labels: Vec<String> = (0..=4)
        .map(|i| format_bytes((i as f64 * step) as u64))
        .collect();

    let title = if let Some(svc) = &app.focused_service {
        format!(" Memory (RSS) \u{2014} {} ", svc)
    } else if !app.toggled_services.is_empty() {
        let count = app.toggled_services.len();
        format!(" Memory (RSS) \u{2014} {} selected ", count)
    } else {
        " Memory (RSS) \u{2014} All ".to_string()
    };
