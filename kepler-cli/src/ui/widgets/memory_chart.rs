...
use crate::ui::widgets::utils::{chart_data_points, format_bytes, format_duration_label, format_timestamp_label};
...
pub fn render(f: &mut Frame, area: Rect, app: &TopApp) {
    ...
    let mut max_y: f64 = 1024.0 * 1024.0;
    ...
    if app.is_all_view() && !app.services.is_empty() {
        let total_history = app.compute_total_history();
        let points = chart_data_points(&total_history, window, right_edge_ms, |e| e.memory_rss as f64);
        for &(_, y) in &points {
            if y > max_y {
                max_y = y;
            }
    ...
    max_y = (max_y * 1.2).max(1024.0 * 1024.0);
