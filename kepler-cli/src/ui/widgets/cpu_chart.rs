...
use crate::ui::widgets::utils::{chart_data_points, format_duration_label, format_timestamp_label};
...
pub fn render(f: &mut Frame, area: Rect, app: &TopApp) {
    ...
    if app.is_all_view() && !app.services.is_empty() {
        ...
        let points = chart_data_points(&total_history, window, right_edge_ms, |e| e.cpu_percent as f64);
        for &(_, y) in &points {
            if y > max_y {
                max_y = y;
            }
    ...
    max_y = (max_y * 1.2).max(10.0);
    let y_step = if max_y > 50.0 { 25.0 } else { 10.0 };
    max_y = (max_y / y_step).ceil() * y_step;
    ...
    let y_labels: Vec<String> = (0..=((max_y / y_step) as u64))
        .map(|i| format!("{}", (i as f64 * y_step) as u64))
        .collect();
