...
fn render_action_button(
    ...
    let rect = Rect::new(x, y, width, Button::height());
    let hovered = mouse_pos.0 >= rect.x && mouse_pos.0 < rect.x + rect.width
        && mouse_pos.1 >= rect.y && mouse_pos.1 < rect.y + rect.height;
    ...
    let btn = Button::new(&content)
        ...
        .bold(enabled)
        .hovered(hovered)
        .hover_bg(ratatui::style::Color::Reset)
        .hover_border_fg(active_color);
