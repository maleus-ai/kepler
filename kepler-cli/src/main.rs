...
async fn run() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Top { service, json, history, interval } => {
            if json {
                handle_top_json(&client, canonical_path, service, history).await?;
            } else {
                ...
                ui::pages::top::run(&client, canonical_path, service, history_duration, interval).await?;
            }
        }
...
async fn handle_top_json(
    ...
    let (_progress_rx, response_future) = client.monitor_metrics(
        config_path, service, since, None,
    )?;
    let response = response_future.await?;
