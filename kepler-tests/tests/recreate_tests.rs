...
async fn test_restart_calls_restart_hooks() {
    ...
    orchestrator
        .start_services(&config_path, None, Some(sys_env.clone()), None, None, false)
        .await
        .unwrap();
...
async fn test_restart_preserves_baked_config() {
    ...
    orchestrator
        .start_services(&config_path, None, Some(sys_env), None, None, false)
        .await
        .unwrap();
...
async fn test_restart_specific_service_hooks() {
    ...
    // Restart only svc1
    orchestrator
        .restart_services(&config_path, &["svc1".to_string()], false)
        .await
        .unwrap();
