//! Tests for ${{ service.env.VAR }}$ expansion in all config values
//!
...
async fn test_env_file_expansion_in_environment() {
    ...
    let config = TestConfigBuilder::new()
        .add_service(
            ...
            // This ${{ service.env.DB_HOST }}$ and ${{ service.env.DB_PORT }}$ should be expanded from the env_file
            .with_environment(vec!["DATABASE_URL=postgres://${{ service.env.DB_HOST }}$:${{ service.env.DB_PORT }}$/mydb".to_string()])
            .build(),
        )
        .build();
...
async fn test_user_expansion() {
    ...
    let config = TestConfigBuilder::new()
        .add_service(
            ...
            .with_user("${{ service.env.KEPLER_TEST_USER }}$")
            .build(),
        )
        .build();
...
/// Default value syntax ${{ service.env.VAR or "default" }}$ works
#[tokio::test]
async fn test_default_value_syntax() {
    ...
    let config = TestConfigBuilder::new()
        .add_service(
            ...
            // ${{ service.env.KEPLER_UNDEFINED_VAR or "default_path" }}$ should expand to "default_path"
            .with_environment(vec![
                "DEFAULT_VAL=${{ service.env.KEPLER_UNDEFINED_VAR or \"default_path\" }}$".to_string(),
            ])
            .build(),
        )
        .build();
...
async fn test_env_file_overrides_system_for_expansion() {
    ...
    let config = TestConfigBuilder::new()
        .add_service(
            ...
            .with_environment(vec!["EXPANDED=${{ service.env.KEPLER_OVERRIDE_VAR }}$".to_string()])
            .build(),
        )
        .build();
...
async fn test_env_file_persists_after_deletion() {
    ...
    // Start and take snapshot (autostart: true enables snapshot persistence)
    let harness = TestDaemonHarness::new(config, temp_dir.path())
        .await
        .unwrap();
...
async fn test_independent_env_files() {
    ...
    let config = TestConfigBuilder::new()
        .add_service(
            ...
            .with_environment(vec!["EXPANDED=${{ service.env.SERVICE_NAME }}$".to_string()])
            .build(),
        )
        .add_service(
            "service_b",
...
async fn test_working_dir_expansion() {
    ...
    let config = TestConfigBuilder::new()
        .add_service(
            ...
            .with_working_dir(std::path::PathBuf::from("${{ service.env.KEPLER_WORK_DIR }}$"))
            .build(),
        )
        .build();
